...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(cmd, "mode") == 0) {
    ...
    if (strcmp(target, "bh") == 0) ledStates[0] = (state == 1);
    else if (strcmp(target, "temp") == 0) ledStates[1] = (state == 1);
    else if (strcmp(target, "humi") == 0) ledStates[2] = (state == 1);
    ...
    if (strcmp(target, "temp") == 0) readTemp = (state == 1);
    else if (strcmp(target, "humi") == 0) readHumi = (state == 1);
    else if (strcmp(target, "lux") == 0) readLux = (state == 1);
    else if (strcmp(target, "all") == 0) sensorsEnabled = (state == 1);
...
void updateWave() {
  ...
  if (currentMillis - lastWaveTime >= waveSpeed) {
    ...
    else if (currentLedStep == 1) digitalWrite(LED_TEMP, HIGH);
    else if (currentLedStep == 2) digitalWrite(LED_HUMI, HIGH);
...
void reconnectMQTT() {
  ...
  if (!client.connected() && WiFi.status() == WL_CONNECTED) {
    if (currentMillis - lastReconnectAttempt >= reconnectInterval) {
      ...
      if (client.connect("ESP32_Dashboard_Client", mqtt_user, mqtt_pass, TOPIC_STATUS, 0, true, "{\"status\":\"offline\",\"trigger\":\"lwt\"}")) {
...
void checkResetReason(int core) {
  ...
  switch (reason) {
    case 1: Serial.println(F("POWERON_RESET")); break;
    case 3: Serial.println(F("SW_RESET")); break;
    case 4: Serial.println(F("OWDT_RESET")); break; // Watchdog
    case 12: Serial.println(F("SW_CPU_RESET")); break;
    case 14: Serial.println(F("TG0WDT_SYS_RESET")); break; 
    case 15: Serial.println(F("BROWNOUT_RESET")); break; // Sụt nguồn
    case 16: Serial.println(F("RTCWDT_RTC_RESET")); break;
...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  WiFi.setTxPower(WIFI_POWER_15dBm); // Giảm peak current, tránh brownout trên USB
