...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  } else if (strstr(msg, "temp_on")) {
    readTemp = true;
    publishStatus("cmd_temp_on");
  } else if (strstr(msg, "temp_off")) {
    readTemp = false;
    publishStatus("cmd_temp_off");
  } else if (strstr(msg, "humi_on")) {
    readHumi = true;
    publishStatus("cmd_humi_on");
  } else if (strstr(msg, "humi_off")) {
    readHumi = false;
    publishStatus("cmd_humi_off");
  } else if (strstr(msg, "lux_on")) {
    readLux = true;
    publishStatus("cmd_lux_on");
  } else if (strstr(msg, "lux_off")) {
    readLux = false;
    publishStatus("cmd_lux_off");
...
void updateSensors() {
  ...
  if (currentMillis - lastSensorTime >= sensorInterval) {
    ...
    // Validate only if we're actually trying to read them
