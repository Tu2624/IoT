  ...
  <div class="page">
    ...
    <div class="filter-row">
      <div class="filter-left">
        ...
        <span class="filter-label">Lọc dữ liệu</span>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="row in paginated" :key="row.id" class="table-row">
            ...
            <td>{{ row.name }}</td>
            <td>
              <span class="badge" :class="row.status === 'On' ? 'badge-on' : 'badge-off'">
                {{ row.status }}
              </span>
            </td>
...
function randomTime(id) {
  const now = new Date()
  now.setMinutes(now.getMinutes() - id * 2)
  const hh = String(now.getHours()).padStart(2,'0')
  const mm = String(now.getMinutes()).padStart(2,'0')
  const dd = String(now.getDate()).padStart(2,'0')
  const mo = String(now.getMonth()+1).padStart(2,'0')
  return `${hh}:${mm}-${dd}/${mo}/${now.getFullYear()}`
}
const allRows = ref(Array.from({ length: 120 }, (_, i) => ({
  id: i + 1,
  name: sensorTypes[i % 3],
  status: i % 3 === 0 ? 'Off' : i % 5 === 0 ? 'Off' : 'On',
  time: randomTime(i)
})))
...
const filtered = computed(() => {
  ...
  rows = [...rows].sort((a, b) => sortAsc.value ? a.id - b.id : b.id - a.id)
  return rows
})
...
function exportCsv() {
  const header = 'ID,Tên cảm biến,Giá trị,Thời gian\n'
  const body = filtered.value.map(r =>
    `${r.id},"${r.name}",${r.status},"${r.time}"`
  ).join('\n')
...
.btn-sort:active { transform: scale(0.96); box-shadow: none; transition: transform 0.08s ease; }
...
.table-row { transition: background 0.2s ease, transform 0.15s ease; }
...
.td-id   { font-weight: 700; color: rgba(255,255,255,0.6); }
.td-time { color: rgba(255,255,255,0.7); font-size: 14px; }
